...
def main():
    ...
    cubic_spline = CubicSpline(
        updated_index_block, updated_data_block, bc_type='natural')
    ...
    try:
        ...
        for currentArgument, currentValue in arguments:
            ...
            if currentArgument in ("-h", "--help"):
                ...
                # Read track here
                fs_cubic_clean, cubic_clean_track = readTrack(
                    "clean_cubic.wav")
                ...
                print(
                    "Displaying the MSE error difference between the two restored tracks")
                # Read track here
                fs_cubic_clean, cubic_clean_track = readTrack(
                    "clean_cubic.wav")
                fs_median_clean, median_clean_track = readTrack(
                    "clean_median.wav")
                fs_original_clean, original_clean_track = readTrack(
                    "new_clean.wav")
                ...
                plotGraph(cubic_clean_track, fs_cubic_clean,
                          "Restored Track using Cubic Interpolation")

                print("Plotting graph for the original track")
                plotGraph(original_clean_track,
                          fs_original_clean, "Original Track")
